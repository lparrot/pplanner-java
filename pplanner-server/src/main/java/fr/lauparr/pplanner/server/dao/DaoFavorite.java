...
public interface DaoFavorite extends JpaRepository<Favorite, String>, JpaSpecificationExecutor<Favorite> {
	...
	@Query("select f from Favorite f where f.user.id = :userId and f.menuItem.project.id = :projectId and f.deletedAt is null")
	List<Favorite> findAllFavoriteByUserIdAndProjectId(@Param("userId") String userId, @Param("projectId") String projectId);

	@Query("select case when count(f)> 0 then true else false end from Favorite f where f.user.id = :userId and f.menuItem.id = :menuId and f.deletedAt is null")
